from getBlockInfo import getBlockInfo, getFbFromDb
...
def analysis_one_block(block_num, fbtx_num, block_gas_price):
    print(f"block_num: {block_num}", fbtx_num)
    # 从 flashbot api 获取fb交易信息
    # 将fb交易信息输出到明细表格中
    fb_data = getFbFromDb(block_num)
    fb_data = fb_data.sort_values(axis=0, by='bundle_index', ascending=True)
    # print(fb_data)
    ...
    for i in range(fbtx_num):
        fbi = fb_data.iloc[i]
        # print("lalala", i, fbi['bundle_index'])
    ...
    for i in range(fbtx_num):
        fb_df = fb_df.copy()
        idx = int(fb_data.iloc[i]['bundle_index'])
        # print("idx:", idx)
        fb_df.loc[i, ['buddle_price']] = buddle_gasPrice[idx]
    # print(fb_df)
    ...
    if fb_lastBuddle_price >= nfb_max:
        return [[block_num, block_len, block_gas_price/1e9, fb_min/1e9, fb_min_bubbleIndex,
        ...
        default_list = []
        for i in range(fbtx_num):
            if fb_data.iloc[i]['bundle_index'] == buddle_index:
                default_list.append(fb_data.iloc[i]['transaction_hash'])
...
if __name__ == '__main__':
    df_out = pd.DataFrame(columns=['block_num', 'block_len', 'block_gas_price', 'fb_min_price', 'fb_min_buddleIndex',
    ...
    block_info = getBlockInfo(2000)
    for i in range(len(block_info)):
        print(f"\n第{i}个block:")
        res = analysis_one_block(int(block_info[i][0]), int(block_info[i][1]), int(block_info[i][2]))
        if i == 0:
